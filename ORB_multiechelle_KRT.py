...
DETECT_EVERY_N_FRAMES_NO_TRACK = 3
DETECT_EVERY_N_FRAMES_TRACKING = 60
MIN_MATCHES = 15
RATIO_THRESHOLD = 0.7
BEST_THRESHOLD = 15
FILTRAGE = 0
ROI = 1
HEADLESS = True  # ← NOUVELLE OPTION: True = pas d'affichage, False = affichage normal
...
class PosterTracker:
    def __init__(self, poster_data, gray_frame):
        ...
        self.corners = self._order_corners(poster_data['corners'])
        self.poster_data = poster_data
        self.poster_data['corners'] = self.corners
    ...
    def update(self, prev_gray, curr_gray):
        ...
        self.corners = self._order_corners(new_corners)
        self.poster_data['corners'] = self.corners
...
if __name__ == '__main__':
    ...
    # DataLoader
    print("Chargement du DataLoader...")
    loader = DataLoader("config.json")
    
    # Initialisation des CSV (un par affiche, tous sujets confondus)
    os.makedirs("gaze_points", exist_ok=True)
    csv_files = {}
    file_handles = {}
    
    for poster_name in templates.keys():
        csv_path = f"gaze_points/{poster_name}.csv"
        file_exists = os.path.exists(csv_path)
        fh = open(csv_path, "a", newline="")
        writer = csv.writer(fh)
        if not file_exists:
            writer.writerow(["subject_id", "x_aff", "y_aff"])  # ← Ajout de subject_id
        csv_files[poster_name] = writer
        file_handles[poster_name] = fh
    
    # Statistiques globales
    global_stats = {name: {} for name in templates.keys()}  # {poster: {subject: count}}
    
    # Affichage du mode
    mode_text = "MODE HEADLESS" if HEADLESS else "MODE AVEC AFFICHAGE"
    print(f"\n{mode_text}")
    print(f"Sujets à traiter: {SUBJECTS}\n")
    
    # BOUCLE SUR LES SUJETS
    for subject_idx in SUBJECTS:
        print(f"\n{'='*60}")
        print(f"TRAITEMENT DU SUJET {subject_idx}")
        print(f"{'='*60}")
        ...
        try:
            ...
            print(f"ERREUR lors du chargement des données pour le sujet {subject_idx}: {e}")
            continue
        ...
        # Variables de suivi pour ce sujet
        frame_index = 0
        active_trackers = []
        last_detected = []
        gaze_stats = {}
        ...
        for _ in frame_iterator:
            ...
            # Créer annotated seulement si pas en mode headless
            annotated = None if HEADLESS else frame_und.copy()
                ...
                if tracker.is_valid():
                    if check_gaze_on_poster(x, y, tracker.corners):
                        ...
                        if gx_aff is not None:
                            csv_files[tracker.name].writerow([subject_idx, gx_aff, gy_aff])  # ← Ajout subject_id
                            gaze_stats[tracker.name] = gaze_stats.get(tracker.name, 0) + 1
                        break
            
            # Visualisation (seulement si pas headless)
            if not HEADLESS:
                for tracker in active_trackers:
                    if tracker.is_valid():
                ...
                status_text = f"Sujet {subject_idx} | Frame: {frame_index} | Trackers: {len(active_trackers)} | Prochaine détection: {next_detect}"
        ...
        print(f"\n--- Résumé Sujet {subject_idx} ---")
        print(f"Frames traitées: {frame_index}")
        for name, count in gaze_stats.items():
            print(f"  - {name}: {count} points de regard")
            if name not in global_stats:
                global_stats[name] = {}
            global_stats[name][subject_idx] = count
    ...
    print("RÉSUMÉ GLOBAL")
    print(f"{'='*60}")
    print(f"Sujets traités: {SUBJECTS}")
    print(f"\nFichiers CSV générés dans 'gaze_points/' (avec colonne subject_id):\n")
    
    for poster_name in sorted(global_stats.keys()):
        total = sum(global_stats[poster_name].values())
        print(f"  {poster_name}: {total} points au total")
        for subj in sorted(global_stats[poster_name].keys()):
            count = global_stats[poster_name][subj]
            print(f"    └─ Sujet {subj}: {count} points")
